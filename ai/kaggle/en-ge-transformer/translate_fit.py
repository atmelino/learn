...
def preprocess(english, german):
    english = english_vectorization(english)
    german = german_vectorization(german)
    return (
        {"encoder_inputs": english, "decoder_inputs": german[:, :-1]},
        german[:, 1:],
    )
...
def make_dataset(df, batch_size, mode):
    dataset = tf.data.Dataset.from_tensor_slices(
        (list(df["english"]), list(df["german"]))
    )
    if mode == "train":
...
def get_transformer(config):
    ...
    encoder_outputs = TransformerEncoder(
        config.embed_dim, config.num_heads, config.latent_dim
    )(x)
    encoder = keras.Model(encoder_inputs, encoder_outputs)

    decoder_inputs = keras.Input(shape=(None,), dtype="int64", name="decoder_inputs")
    encoded_seq_inputs = keras.Input(
        shape=(None, config.embed_dim), name="decoder_state_inputs"
    )
    x = PositionalEmbedding(
        config.sequence_length, config.vocab_size, config.embed_dim
    )(decoder_inputs)
    x = TransformerDecoder(config.embed_dim, config.latent_dim, config.num_heads)(
...
filename_time = f"acc_{acc[-1]:.3f}_epochs_{config.epochs}_date_{timestr}"
filename_static = "translate_fit"
fullpath = f"{OUTPUT_PATH}{filename_static}"
print("Saving model to ", fullpath)
transformer.save(fullpath + ".keras")

...
if "Pclass" in features:
    x_columns = pd.concat(
        [x_columns, pd.get_dummies(x_columns["Pclass"], prefix="Pclass")], axis=1
    )
    x_columns.drop("Pclass", axis=1, inplace=True)

if "Sex" in features:
    x_columns = pd.concat(
        [x_columns, pd.get_dummies(x_columns["Sex"], prefix="Sex")], axis=1
    )
...
if "SibSp" in features:
    x_columns = pd.concat(
        [x_columns, pd.get_dummies(x_columns["SibSp"], prefix="SibSp")], axis=1
    )
    x_columns.drop("SibSp", axis=1, inplace=True)

if "Parch" in features:
    x_columns = pd.concat(
        [x_columns, pd.get_dummies(x_columns["Parch"], prefix="Parch")], axis=1
    )
...
if "Fare" in features:
    ...
    # print("df_fare_bins\n",df_fare_bins)
    x_columns = pd.concat(
        [x_columns, pd.get_dummies(df_fare_bins["Fare"], prefix="Fare")], axis=1
    )
...
for train, test in kf.split(x, df_train["Survived"].values):
    fold += 1
    print(f"Fold #{fold}")
    if print_fold == True:
        # print(f"  Train: index={train}")
        # print(f"  Test:  index={test}")
        logging.debug(f"  Train: index={train} size={train.shape}")
        logging.debug(f"  Test:  index={test} size={test.shape}")
